export function cardFactory(recipeDatas) {
    ...
    function getCard() {
        ...
        article.className = 'card position-relative p-0 pb-1 overflow-hidden col-lg-4 col-md-6 col-sm-12';
        const a = document.createElement('a');
        article.appendChild(a);
        a.className = 'position absolute top-0 left-0 bottom-0 right-0 text-decoration-none text-dark'
        a.innerHTML =
            `<img src="" class="card-img-top" style="height: 178px; width:100%; background-color: gray;" />
        ...
        <p class= "w-50 text-justify overflow-hidden fs-card h-50 text-truncate text-justify" style="max-height: 100%;">${description}</p></section>`;
        return article
